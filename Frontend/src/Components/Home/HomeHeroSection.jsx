...
const HomeHeroSection = () => {
    ...
    useEffect(() => {
        ...
        tl
            ...
            .to(titleRef.current, { opacity: 1, y: 0, duration: 0.7, ease: "power3.out" }, 0)
            .to(subtitleRef.current, { opacity: 1, y: 0, duration: 0.5, ease: "power3.out" }, 0.4)
            .to(lineRef.current, { width: "220px", duration: 0.7, ease: "power3.out" }, 0.6)
            .to(descriptionRef.current, { opacity: 0.5, y: 0, duration: 0.5, ease: "power3.out" }, 0.8)
            .to(buttonsRef.current, { opacity: 1, y: 0, duration: 0.5, ease: "power3.out" }, 1);
        ...
        if (pb) {
            ...
            pb.addEventListener("mouseenter", () => {
                gsap.to(pb, {
                    scale: 1.02,
                    boxShadow: '0 8px 20px rgba(250, 204, 21, 0.25), 0 4px 12px rgba(250, 204, 21, 0.15)',
        ...
        if (sb) {
            ...
            sb.addEventListener("mouseenter", () => {
                gsap.to(sb, {
                    scale: 1.02,
                    boxShadow: '0 0 15px rgba(250, 204, 21, 0.2), inset 0 0 10px rgba(250, 204, 21, 0.05)',
    ...
    url('./src/Assets/Images/home images/home_hero.jpg')`
            ...
            <div className="relative z-10 w-full px-12 pb-24 md:px-20 md:pb-32">
                <div className="max-w-3xl -ml-2 md:ml-16">
                    ...
                    {/* Decorative Line */}
                    <div ref={lineRef} className="h-1 bg-gradient-to-r from-yellow-400 via-yellow-300 to-transparent mb-4 md:mb-6 rounded-full shadow-lg shadow-yellow-400/20"></div>

                    {/* Tagline */}
                    <p ref={subtitleRef} className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-yellow-200 mb-3 md:mb-4 font-light drop-shadow-lg" style={{ fontFamily: 'Inter, sans-serif', fontWeight: '300' }}>
                        <span className="sm:hidden">Sacred Journeys</span>
                        <span className="hidden sm:inline">Your Gateway to Sacred Journeys</span>
                    </p>

                    {/* Description */}
                    <p ref={descriptionRef} className="text-gray-300 mb-8 md:mb-12 text-sm sm:text-base leading-relaxed max-w-2xl opacity-70 italic drop-shadow-md" style={{ fontFamily: 'Inter, sans-serif', fontWeight: '400' }}>
                        <span className="sm:hidden">Ancient traditions meet modern comfort for every pilgrim's sacred calling.</span>
                        <span className="hidden sm:inline">Journey with us through the paths of faith, where ancient traditions meet modern comfort and every pilgrim finds their sacred calling.</span>
                    </p>

                    {/* Buttons */}
                    <div ref={buttonsRef} className="flex flex-col sm:flex-row gap-4 md:gap-6" style={{ fontFamily: 'Inter, sans-serif' }}>
                        <button
