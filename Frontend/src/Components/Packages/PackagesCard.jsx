...
function getHighlights(packageData) {
  ...
  if (packageData.hotel)
    highlights.push({ icon: icons.hotel, text: packageData.hotel });
  if (packageData.transport)
    highlights.push({ icon: icons.car, text: packageData.transport });
  if (packageData.ziyarat)
    highlights.push({
      icon: icons.map,
      text: `Ziyarat: ${packageData.ziyarat}`,
    });
  if (packageData.visa)
    highlights.push({ icon: icons.shield, text: `Visa: ${packageData.visa}` });
  if (packageData.makkah)
    highlights.push({ icon: icons.map, text: `Makkah: ${packageData.makkah}` });
  if (packageData.madinah)
    highlights.push({
      icon: icons.map,
      text: `Madinah: ${packageData.madinah}`,
    });
  return highlights;
}

function PackageDialog({ packageData, isOpen, onClose, onBookPackage }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const y = Math.abs(parseInt(scrollY || "0", 10));
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(125% 125% at 50% 10%, #ffffff 40%, #b1b1b1 100%)",
      }}
      onClick={onClose}
    >
      <div
          ...
          <div className="p-6">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-600 leading-relaxed mb-6">
                ...
                package. This comprehensive Umrah package includes everything
                you need for a spiritual and comfortable pilgrimage to the holy
          ...
          <div className="p-5 bg-gradient-to-b from-white to-[#C9A227]/5 flex flex-col h-[calc(100%-12rem)]">
            ...
            <div className="grid grid-cols-1 gap-1.5 mb-4 text-sm flex-grow">
              {highlights.map((highlight, idx) => (
                ...
                >
                  <span className="text-[#C9A227] flex-shrink-0">
                    {highlight.icon()}
                  </span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mt-10">
            <p className="text-gray-600 mb-6 text-lg">
              Need a custom package? We're here to help create your perfect
              journey.
            </p>
            <button className="bg-gradient-to-r from-white to-[#C9A227]/10 border-2 border-[#C9A227] text-[#C9A227] hover:bg-gradient-to-r hover:from-[#C9A227] hover:to-[#DAB83D] hover:text-white hover:border-[#DAB83D] px-8 py-3 rounded-xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl">
