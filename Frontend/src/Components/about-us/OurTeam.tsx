...
function OurTeam() {
  ...
  const [order, setOrder] = useState<TeamMember[]>(team);
  ...
  // duplicates.
  const [pending, setPending] = useState<PendingDirection>(null); // "next" | "prev" | null
                  ...
                  <div className="group rounded-2xl border bg-card p-4 shadow-sm transition hover:shadow-md">
                    <div className="relative h-56 w-full overflow-hidden rounded-xl bg-transparent">
                      <Image
                        ...
                        alt={m.name}
                        fill
                        className="bg-transparent object-cover transition-transform duration-300 group-hover:scale-[1.03]"
                      />
                    </div>
