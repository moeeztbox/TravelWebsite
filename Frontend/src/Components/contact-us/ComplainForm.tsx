...
import { sanitizeDigits, validateCommonFields, type CommonFieldsErrors } from "../../utils/formValidation";
...
function ComplainForm() {
  ...
  const [status, setStatus] = useState<ComplainStatus>({ type: "", message: "" });
  const [sending, setSending] = useState(false);
  const [errors, setErrors] = useState<CommonFieldsErrors>({});

  const [focused, setFocused] = useState<ComplainFocused>({
    firstName: false,
    lastName: false,
    email: false,
    phone: false,
  ...
  const handleFocus = (field: keyof ComplainFocused) => {
    setFocused({ ...focused, [field]: true });
  };

  const handleBlur = (field: keyof ComplainFocused) => {
