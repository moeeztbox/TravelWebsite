...
function Reviews() {
  ...
  const [status, setStatus] = useState<ReviewStatus>({ type: "", message: "" });
  const [hoverRating, setHoverRating] = useState(0);
  const [submitting, setSubmitting] = useState(false);
  const [focused, setFocused] = useState<ReviewFocused>({
    fullName: false,
    email: false,
    phone: false,
    review: false,
  ...
  const handleFocus = (field: keyof ReviewFocused) => {
    setFocused((prev) => ({ ...prev, [field]: true }));
  };

  const handleBlur = (field: keyof ReviewFocused) => {
    setFocused((prev) => ({ ...prev, [field]: false }));
  };

  const validateForm = (): ReviewFormErrors => {
  ...
  const inputBorderClass = (field: keyof ReviewFocused, hasError?: string) => {
