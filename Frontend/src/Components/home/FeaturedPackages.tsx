...
interface CardPackage extends Omit<Package, "highlights"> {
  highlights: CardHighlight[];
}

function mapApiToCardPackage(p: Package): CardPackage {
  return {
    ...p,
...
interface PackageCardProps {
  pkg: CardPackage;
  onBook?: (pkg: CardPackage) => void;
}

const PackageCard = ({ pkg, onBook }: PackageCardProps) => {
  return (
    <motion.div
    ...
    >
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl border border-gray-100 hover:border-[#C9A227]/30 transition-all duration-500 hover:scale-101 h-full">
        <div className="relative h-48 overflow-hidden">
          ...
          <Image
            src={pkg.image || "https://images.unsplash.com/photo-1591604466107-ec97de577aff?w=800"}
            alt={pkg.title}
            fill
            unoptimized
