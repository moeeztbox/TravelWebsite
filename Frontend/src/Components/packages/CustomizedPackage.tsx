...
interface CustomizePackageForm {
  fullName: string;
  city: string;
  phone: string;
  startDate: string;
...
function CustomizePackage() {
  ...
  const [errors, setErrors] = useState<CommonFieldsErrors>({});
   ...
   * `firstName`/`lastName`/`city`/`phone` were never actually present at runtime (this
   * pre-fill silently no-ops today). Kept as a locally-widened read rather than inventing
   * those fields on the shared `AuthUser` type, matching the "don't invent backend
   * behavior" migration rule — same pre-existing gap noted for `createDraftBooking`.
   */
  ...
  const [form, setForm] = useState<CustomizePackageForm>(() => ({
    fullName:
