...
function emptyForm(): AdminPackageForm {
  return {
    packageId: "",
    order: 0,
    title: "",
...
function pkgToForm(pkg: Package): AdminPackageForm {
  return {
...
function AdminPackagesContent() {
  ...
  const [form, setForm] = useState<AdminPackageForm>(emptyForm);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const msg = axiosErr.response?.data?.message || "Save failed";
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const axiosErr = err as AxiosError<ApiErrorPayload>;
      const msg = axiosErr.response?.data?.message || "Delete failed";
      toast.error(msg);
    }
  };
